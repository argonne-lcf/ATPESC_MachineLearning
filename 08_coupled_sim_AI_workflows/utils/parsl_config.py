...
# Parsl config for both a CPU and GPU workers
gpu_bind_cores = {1, 9, 17, 25, 33, 41, 53, 61, 69, 77, 85, 93}
cpu_affinity = "list"
for cpu in range(num_cpu_sockets):
    core_start = cpu * 52
    for core in range(num_cores_per_cpu):
        c = core_start + core + 1
        if c not in gpu_bind_cores:
            cpu_affinity += f":{c}"
...
aurora_config = Config(
    ...
    executors=[
        ...
        HighThroughputExecutor(
            ...
            max_workers_per_node=12, # We will use 12 workers, one for each tile
            # This gives optimal binding of threads to GPUs on an Aurora node
            cpu_affinity="list:1:9:17:25:33:41:53:61:69:77:85:93",
            prefetch_capacity=0,  # Increase if you have many more tasks than workers
            provider=LocalProvider(
                launcher=MpiExecLauncher(
                    bind_cmd="--cpu-bind", overrides="--ppn 1"
