...
if __name__ == "__main__":
    ...
    while len(train_data) <= max_training_count:
        ...
        pool = Pool(policy=[p for p in gpu_policy for _ in range(num_nodes)],    # Dragon round-robins across nodes first
            processes_per_policy=1, 
            initializer=setup, 
            initargs=(dd,)
        )
        ...
        num_workers = min(num_cores_per_node * num_nodes, batch_size)
        pool = Pool(num_workers)
