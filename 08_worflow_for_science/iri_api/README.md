...
Retrieves an access token and submits a short job to Polaris via a `POST` to the compute endpoint. The example runs `echo Start; sleep 10; echo End` on one node under the `debug` queue and `ATPESC2026` account, writing stdout/stderr to `$HOME/logs`.
