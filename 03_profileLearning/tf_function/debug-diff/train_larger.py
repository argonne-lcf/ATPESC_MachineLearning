...
class MNISTClassifier(tf.keras.models.Model):
    ...
    def __init__(self, activation=tf.nn.tanh):
        ...
        self.conv_1 = tf.keras.layers.Conv2D(32, [5, 5], activation='relu')
        self.conv_2 = tf.keras.layers.Conv2D(64, [5, 5], activation='relu')
        self.conv_2a = tf.keras.layers.Conv2D(128, [5, 5], activation='relu',
                                             padding='same')
        self.conv_2b = tf.keras.layers.Conv2D(512, [3, 3], activation='relu', padding='same')
        self.pool_3 = tf.keras.layers.MaxPooling2D(pool_size=(2, 2))
        self.drop_4 = tf.keras.layers.Dropout(0.25)
        self.conv_temp = tf.keras.layers.Conv2D(128, [5, 5], activation='relu')
        # self.dense_5 = tf.keras.layers.Dense(128, activation='relu')
        self.dense_5 = tf.keras.layers.Dense(4096, activation='relu')
